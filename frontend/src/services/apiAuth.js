...
import { setAccessToken, getAccessToken, deleteTokenFromVariable } from './auth.js';
...
apiAuth.interceptors.response.use((res) => {
    ...
    async (error) => {
        ...
        if (error.response.status && error.response.status === 401 && !originalRequest._retry) {
            ...
            try {
                const new_token = await axios.post('http://localhost:8000/api/token/refresh-cookie/', {}, {withCredentials: true});
